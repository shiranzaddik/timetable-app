  ...
  return (
    <div className="form-card">
      ...
      <div className="form-row">
        ...
        >
          {/* "No trend" is always the first option so an empty school still
              shows a meaningful default instead of a phantom "מגמה ג". When
              the user has set up real trends for the grade they appear
              below; "+ New trend" is always available at the end. */}
          <option value="">{t("noTrendOption")}</option>
          {(trendsByGrade[grade] ?? []).filter(Boolean).map((name) => (
